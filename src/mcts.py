...
class MCTSNode:
    def __init__(self,
        ...
        self.P = np.ones(n_actions) / n_actions if p is None else p # TODO this is misleading as default value is not visible in function default arguments
        self.R = [None]*self.n_actions # TODO why is this not np.array
        self.S = [None]*self.n_actions

    def add_child(self, child, a=0, r=0):
    ...
    def selection(self, transform="qtransform_by_parent_and_siblings", rng_key=jax.random.PRNGKey(42)):
        if transform == "qtransform_by_parent_and_siblings":
            ...
            safe_qvalues = np.where(self.N > 0, self.Q, self.v)
            min_value = np.minimum(self.v, np.min(safe_qvalues))
            max_value = np.maximum(self.v, np.max(safe_qvalues))
            completed_by_min = np.where(self.N > 0, self.Q, min_value)
            Q_scaled = (completed_by_min - min_value) / (np.maximum(max_value - min_value, 1e-8))
        else:
            # Naive implementation:
            assert len(transform) == 2
            Q_scaled = (self.Q - transform[0]) / (transform[1] - transform[0])
        
        pucb = Q_scaled + self.P * np.sqrt(self.N.sum()) / (1 + self.N) * (_c1 + np.log( (self.N.sum() + _c2 + 1)/_c2 ))
        noise = 1e-7 * np.array(jax.random.uniform(rng_key, shape=pucb.shape))
        action = np.argmax(pucb + noise)
        next_node = self.S[action] # could be None
        reward = self.R[action] # could also be None
...
class MCTSTree:
    def __init__(self,
                 root_node=None,
                 n_simulations=50,
                 dynamics_function = lambda s, a: (0.0, s),
                 prediction_function = lambda s: (None, 0.0),
                 gamma=0.997,
                 n_actions=18,
    ...
    def backup(self):
        ...
        leaf_node = last_non_leaf_node.S[a]
        ...
        for (a, node, r) in reversed(self.trace):
            ...
            node.Q[a] = (node.N[a] * node.Q[a] + G) / (node.N[a] + 1)
            node.N[a] +=1
            node.n_visits += 1
    ...
    def sample(self):
        ...
        return np.random.choice(self.n_actions, p=self.root_node.N/self.root_node.N.sum())
